...
def loadTierMeta():
    ...
    # Todo Dont forget to reference CAIDA and report publication according to the readme in the dir of file above
    ...
    if not os.path.isfile("data/"+new_filename+".csv"):
        log("loadTierMeta", "DEBUG", "File doesnt exist local, downloading now!")
    ...
    log("loadTierMeta", "DEBUG", "Tier 1: "+str(tier1)+" Tier 2: "+str(tier2)+" Tier 3: "+str(tier3)+" Tier X: "+str(tierx))
...
def getTier(asn):
    try:
        ...
        log("getTier", "DEBUG", "Could not open tiermeta.json, downloading new one")
